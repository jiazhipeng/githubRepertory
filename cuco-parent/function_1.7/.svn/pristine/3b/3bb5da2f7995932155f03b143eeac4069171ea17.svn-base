    ...
    <!--状态-->
   	...
   	<section class="upload">
        ...
        <div class="wrapper wrapper_one">
           <c:if test="${null==memberInfo.license || ''==memberInfo.license}">
           		<img onclick="editPicture('license')" id="licenseImg" class="license">
           </c:if>
           <c:if test="${null!=memberInfo.license && ''!=memberInfo.license}">
           		<img onclick="editPicture('license')" id="licenseImg" src="${memberInfo.license}">
           </c:if>
           
        </div>
    </section>
    <section class="upload">
        <div class="title">上传税务登记证</div>
        <div class="wrapper wrapper_one">
        	<c:if test="${null==memberInfo.taxReg || ''==memberInfo.taxReg}">
           		<img onclick="editPicture('taxReg')" id="taxRegImg" class="taxreg">
           </c:if>
           <c:if test="${null!=memberInfo.taxReg && ''!=memberInfo.taxReg}">
           		<img onclick="editPicture('taxReg')" id="taxRegImg" src="${memberInfo.taxReg}">
           </c:if>
           
           <input id="taxReg" type="hidden" value=""/>
           <input id="license" type="hidden" value=""/>
...
var basePath = "${ctx}";
	...
	function upload(){
		...
		var pictures = {};
		if(license){
			pictures["license"] =  license;
			
		}
		
		if(taxReg){
			pictures["taxReg"] =  taxReg;
		}
...
function formSubmit(){
	...
	if(2==status){
		...
		if(!"${memberInfo.license}" && !$("#license").val()){
			alert("请上传营业执照");
			return false;
		}
		if(!"${memberInfo.taxReg}" && !$("#taxReg").val()){
			alert("请上传税务登记证");
			return false;
		}
