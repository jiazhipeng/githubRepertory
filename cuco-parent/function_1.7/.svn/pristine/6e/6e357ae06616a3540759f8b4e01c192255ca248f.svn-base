...
$.extend($.fn.fmatter, {
   ...
   mydatetime2 :function(cellValue, options) {
       if(cellValue) {
           return $.fmatter.util.DateFormat(
               '', 
               new Date(cellValue), 
...
//						if(parseFloat($($parent).css("top"))<62){
//							$($parent).css("top","62px");
//						}
					} 
				}
		...
		form_dialog_forOnBeforeClose : function(gridid,p,ajaxso){
			p =  $.extend({
				width:500,
				height:400, 
				autoOpen: false,
...
//						}
	 ...
	 htmlPoaitions:function(width,height,id){
		 var curleft = 0, curtop = 0;
		if (typeof window.innerWidth != 'undefined') {
			curleft = window.innerWidth;
			curtop = window.innerHeight;
	...
	//	alert(document.getElementById(id).offsetTop);
		...
		return [curleft/2-width/2,document.getElementById(id).offsetTop];
	 }
