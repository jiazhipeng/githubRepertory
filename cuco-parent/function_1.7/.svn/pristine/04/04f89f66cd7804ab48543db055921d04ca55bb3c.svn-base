    ...
    <div class="charge-page">
        <ul class="charge-t">
            ...
            <li class="down" id="status_li" onclick="showStatus(this)">
                ...
                <p class="state-list">
                    ...
                    <a href="javascript:setStatus(6,this)">已失联</a>
        ...
        <ul class="charge-list" id="charging_list">
            <c:forEach items="${butlerTaskList}" var="model">
			    ...
			    <li>
			    <a href="${ctx}/wechat/butlerTask/getCar/detail?id=${model.id}&loginId=${loginId}&queryTime=${queryTime}">
...
function setStatus(status,obj){
	...
	$.ajax({
		type: 'POST',
		url: "${ctx}/wechat/butlerTask/getCar/butlerTaskList",
		async:false,
		...
		success: function(data){
			...
			if(null!=data.butlerTaskList){
				for(var i=0;i<data.butlerTaskList.length;i++){
					if(data.butlerTaskList[i].status!=9){
						var mobile = "";
						var operaterName = "";
						if(null!=data.butlerTaskList[i].memberMobile && ""!=data.butlerTaskList[i].memberMobile){
							mobile = data.butlerTaskList[i].memberMobile;
						}
						if(null!=data.butlerTaskList[i].operaterName && ""!=data.butlerTaskList[i].operaterName){
							operaterName = data.butlerTaskList[i].operaterName;
						}
						bulterStr +=("<li>");
						bulterStr +=("<a href='${ctx}/wechat/butlerTask/getCar/detail?id="+data.butlerTaskList[i].id+"&loginId="+$("#loginId").val()+"&queryTime="+$("#queryTime").val()+"'>");
						...
						bulterStr +=("<p class='flex'>"+operaterName+"</p>");
						bulterStr +=("<p class='flex'>"+resolveCharacterDate(data.butlerTaskList[i].planTime,'MM-dd hh:mm')+"</p>");
						bulterStr +=("</a>");
						bulterStr +=("</li>");
					}
				}
