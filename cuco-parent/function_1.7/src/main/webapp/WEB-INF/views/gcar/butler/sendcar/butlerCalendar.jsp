...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<style>
		...
		.butler-name{
			border-bottom:1px solid #ccc; padding:2% 0;
		}
	</style>
</head>
...
<body>
    ...
    <section class="content">
        ...
        <div class="button fixed-btn" onclick='location.href = document.referrer;'>返回</div>

    <%--<c:if test="${loginId eq changeButlerId || empty changeButlerId}">--%>
            <%--<div class="button fixed-btn" onclick='javascript:managementGetCarButler();'>创建任务</div>--%>
        <%--</c:if>--%>
    </section>
    ...
    <script src="${ctx}/static/gcar_html/js/sendCar/carmgr.js?t="+new Date().getTime()></script>
    <script>
    	function searchButler(){
    		//查找管家
    		 var loginId = $("#loginId").val();
             window.location.href=basePath+"/wechat/butler/task/sendCar/toSearchMemberList?loginId="+loginId+"&type=1";
    	}
    	function managementGetCarButler(){
    		//进行跳转
    		var loginId = $("#loginId").val();
            window.location.href=basePath+"/wechat/butler/task/sendCar/toSearchMemberList?loginId="+loginId+"&type=2";
    	}
    </script>
</body>
</html>