...
(function($) {
	 ...
	 var formFieldCreator = {
	 	...
	 	textarea: function(formData, prop, className) {
	 		...
	 		var result = "<td width='120' class='CaptionTD'>" + prop.name + "：</td>";
	 		if(prop.writable === true) {
	 			result +="<td class='DataTD'><textarea  id='" + prop.id + "' name='fp_" + prop.id + "' class='textarea unifyTextareaHeight " + className + " FormElement ui-widget-content ui-corner-all' style='width:180px;height:70px;' >"+val+"</textarea>";
	 		}else{
	 			result += "<td class='DataTD'>" + val;
	 		}	 		
	 		return result;
				...
				if(rowid){						
					$.ajax($.extend({
						...
						success : function (data) {
							if(data.success){
								$.jgrid.info_dialog2('提示',data.message);	
								$("#"+opt.gridId).jqGrid('setRowData',rowid,data.userdata);						
	 ...
	 $.workflow.historic = {
		 ...
		 view : function(gridId,options){
			...
			if(gsr){
				...
				var vw = document.documentElement.clientWidth*0.7;
		 ...
		 showStartupProcessDialog : function (opt) {
			...
			if(rowid){	
			 	...
			 	$('<div/>', {
			 		...
			 		height: 15,
			 		open: function() {			
			 			$.workflow.dynamic.readFormFields.call(this,submitAction,getAction);
			 		},
					close : function(event, ui){
						 $( this ).dialog("destroy");
						 $( this ).remove();
			 				...
			 				if($(".dynamic-form").valid()) {
			 					$('.dynamic-form').ajaxSubmit($.extend( {
			 						...
			 						success : function(data, statusText, xhr, $form) {
			 							if(data.success){
//			 								$.jgrid.info_dialog2('提示',data.message,{zIndex:2000});
			 								$.jgrid.info_dialog2('提示',"启动成功!",{zIndex:2000});
			 								grid.jqGrid('setRowData',rowid,data.userdata);	
		 ...
		 */
		 showTaskDialog : function (opt) {
			...
			if(rowid){	
				...
				var task = $.workflow.task.deserializeTasks(ret.taskinfo);
				if(opt.flag){
					rowid = ret['processInstance.businessKey'];
				}
				 //var vw = document.documentElement.clientWidth * 0.6;
				 //var vh = document.documentElement.clientHeight * 0.5;
			 				...
			 				if($(".dynamic-form").valid()) {
			 					$('.dynamic-form').ajaxSubmit($.extend( {
			 						...
			 						success : function(data, statusText, xhr, $form) {
			 							if(data.success){
//			 								$.jgrid.info_dialog2('提示',data.message,{zIndex:2000});
			 								$.jgrid.info_dialog2('提示',"办理成功!",{zIndex:2000});	
		 ...
		 readFormFields : function (submitAction,getAction) {		 	
		 	...
		 	$.getJSON(getAction, function(data) {
		 		...
		 		$.each(data.formData.formProperties, function() {
		 			...
		 			tagContent += "<tr class='FormData'>" + $.workflow.dynamic.createFieldHtml(data, this, className)
		 			if(this.required === true) {
		 				tagContent += "<span style='color:red'>*</span>";
		 			}
		 			tagContent += "</td></tr>";
		 		});
		 		 
		 		// 添加table内容
		 		$('.dynamic-form-table').html(tagContent).find('tr').hover(function() {
		 			$(this).addClass('ui-state-hover');
		 		}, function() {
		 			$(this).removeClass('ui-state-hover');
		 		});
		 		$('.dynamic-form-table').append(trs);
		 ...
		 if(rowid){	
			 ...
			 var getAction = ctx +'/'+opt.path+'/generic/formkey/get/start/form/' + opt.processDefinitionKey;
			 	...
			 	$('<div/>', {
			 		...
			 		open: function() {			
			 			$.workflow.formkey.readForm.call(this, submitAction,getAction);
			 		},
					close : function(event, ui){
						 $( this ).dialog("destroy");
						 $( this ).remove();
...
//			 								$.jgrid.info_dialog2('提示',data.message,{zIndex:2000});
			 								...
			 								$.jgrid.info_dialog2('提示',"启动 成功!",{zIndex:2000});
		 ...
		 */
		  ...
		  * 读取流程启动表单
		  */
		  readForm : function (submitAction,getAction) {
			 	var dialog = this;
