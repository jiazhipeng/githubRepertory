...
import com.hy.authorization.util.StringUtil;
...
public class User extends PageQuery<User>  implements Serializable {
    ...
    /**
     * 判断用户是否拥有某角色
     * @param roleUniqueCode
     * @return
     */
    public boolean hasRole(String roleUniqueCode){
        if(StringUtils.isEmpty(roleUniqueCode)){
            return false;
        }
        ...
        for(Role role : roles){
            ...
            if(roleUniqueCode.equals(role.getUniqueCode())){
                return true;
            }
        }
        return false;
