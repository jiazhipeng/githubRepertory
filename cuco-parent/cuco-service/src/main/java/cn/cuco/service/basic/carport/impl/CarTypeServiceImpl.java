...
public class CarTypeServiceImpl implements CarTypeService {
	...
	public CarType createCarType(CarType carType) {
		...
		ParamVerifyUtils.paramNotNull(carType.getDeposit(), "创建车型，押金不能为空");
	...
	public CarType updateCarType(CarType carType) {
		...
		ParamVerifyUtils.paramNotNull(carType.getBrand(), "修改车型，车型品牌不能为空");
		ParamVerifyUtils.paramNotNull(carType.getCarType(), "修改车型，车型型号不能为空");
		ParamVerifyUtils.paramNotNull(carType.getTankVolume(), "修改车型，油箱体积不能为空");
		ParamVerifyUtils.paramNotNull(carType.getDeposit(), "修改车型，押金不能为空");
		ParamVerifyUtils.paramNotNull(carType.getImageUrl(), "修改车型，车型图片不能为空");
		ParamVerifyUtils.paramNotNull(carType.getInsuranceUnlock(), "修改车型，解锁保险费不能为空");
		ParamVerifyUtils.paramNotNull(carType.getInsuranceNotUnlock(), "修改车型，非解锁保险费不能为空");
		ParamVerifyUtils.paramNotNull(carType.getIsRestriction(), "修改车型，是否参与限号不能为空");
		ParamVerifyUtils.paramNotNull(carType.getBasicCost(), "修改车型，基础费用不能为空");
	...
	* @return void
	...
	public List<CarType> getCarTypeListByUp() {
	     /*搜索条件*/
	    CarType carTypeSearch = new CarType();
	    carTypeSearch.setValid(Valid.UP.getValue());
		return  this.carTypeMapper.selectByCondition(carTypeSearch);
	}
	...
	public PageResult<CarType> getCarTypeByUpAsc(CarType carType) {
	    ...
	    /*分页信息*/
	    PageHelper.startPage(page,pageSize);
	    carTypes = this.carTypeMapper.selectByConditionByUpAsc(carTypeSearch);
	    if(CollectionUtils.isNotEmpty(carTypes)){
	    	for (CarType carTypeView : carTypes) {
	    		Car car = new Car();
	    		car.setCarTypeId(carTypeView.getId());
	...
	public List<String> getCarBrandForCreateCar() {
		return this.carTypeMapper.selectCarBrandForAll();
	}

	@Override
	public List<CarType> getCartypeByBrandForCreateCar(CarType carType) {
		ParamVerifyUtils.paramNotNull(carType);
		ParamVerifyUtils.paramNotNull(carType.getBrand(), "根据品牌查询车型，品牌不能为空");
		return this.carTypeMapper.selectCartypeByBrand(carType);
	}
