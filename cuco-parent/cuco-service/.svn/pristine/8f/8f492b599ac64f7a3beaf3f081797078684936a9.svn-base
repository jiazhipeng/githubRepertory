...
public class CarServiceImpl implements CarService {
	...
	public Car createCar(Car car) {
		...
		car.setCityId(Long.parseLong("2"));
		car.setCityName("北京市");
	...
	private void vilidateCarInfo(Car car){
		...
		ParamVerifyUtils.paramNotNull(car.getOutTime(),"脱表时间不能为空");
	...
	public Car updateCarByRepairing(Car car) {
		...
		ParamVerifyUtils.paramNotNull(car.getModifierId(),"操作人ID不能为空");
		ParamVerifyUtils.paramNotNull(car.getModifier(),"操作人不能为空");
		car.setRepairdStartTime(new Date());
	...
	private void createCarInsuranceDetailUrl(CarInsuranceDetail carInsuranceDetail){
		//封装要插入的强制险的图片地址list
		List<CarAttachment> carAttachmentList = new ArrayList<>();
		for(int i=0;i<carInsuranceDetail.getCompulsoryInsuranceUrl().length;i++){
			CarAttachment carAttachment = new CarAttachment();
			carAttachment.setParentId(carInsuranceDetail.getId());
			carAttachment.setType(CarEnum.CarAttachmentType.COMPULSORY_INSURANCE_URL.getIndex());
			carAttachment.setUrl(carInsuranceDetail.getCompulsoryInsuranceUrl()[i]);
			carAttachmentList.add(carAttachment);
		}
		//封装要插入的商业险的图片地址list
		List<CarAttachment> bcarAttachmentList = new ArrayList<>();
		for(int i=0;i<carInsuranceDetail.getBusinessInsuranceUrl().length;i++){
			CarAttachment carAttachment = new CarAttachment();
			carAttachment.setParentId(carInsuranceDetail.getId());
			carAttachment.setType(CarEnum.CarAttachmentType.BUSINESS_INSURANCE_URL.getIndex());
			carAttachment.setUrl(carInsuranceDetail.getBusinessInsuranceUrl()[i]);
			bcarAttachmentList.add(carAttachment);
		}
		if(carAttachmentList.size()>0){
			this.carAttachmentService.createCarInsuranceAttachmenBatch(carAttachmentList);
		}
		if(bcarAttachmentList.size()>0){
			this.carAttachmentService.createCarInsuranceAttachmenBatch(bcarAttachmentList);
		}
	}
	...
	public CarInsuranceDetail getCarInsuranceDetailByCarId(Long carId) {
		...
		carAttachment.setParentId(carInsuranceDetail.getId());
		carAttachment.setType(CarEnum.CarAttachmentType.COMPULSORY_INSURANCE_URL.getIndex());
		carAttachmentList = this.carAttachmentService.getCarAttachmentList(carAttachment);
		int isize = carAttachmentList.size();
		String[] compulsoryInsuranceUrl = new String[isize];
		for(int i=0;i<carAttachmentList.size();i++){
			compulsoryInsuranceUrl[i] = carAttachmentList.get(i).getUrl();
		}
		//查询商业险图片
		List<CarAttachment> bcarAttachmentList = new ArrayList<>();
		CarAttachment bcarAttachment = new CarAttachment();
		bcarAttachment.setParentId(carInsuranceDetail.getId());
		bcarAttachment.setType(CarEnum.CarAttachmentType.COMPULSORY_INSURANCE_URL.getIndex());
		bcarAttachmentList = this.carAttachmentService.getCarAttachmentList(bcarAttachment);
		int jsize = bcarAttachmentList.size();
		String[] businessInsuranceUrl = new String[jsize];
		for(int i=0;i<bcarAttachmentList.size();i++){
			businessInsuranceUrl[i] = bcarAttachmentList.get(i).getUrl();
		}
	...
	public Car getCarByCarPlateNum(String carPlateNum) {
		ParamVerifyUtils.paramNotEmpty(carPlateNum,"根据车牌号获取车辆信息失败：车牌号不可为空");
		Car searchCar = new Car();
		searchCar.setCarPlateNum(carPlateNum);
		List<Car> cars = carMapper.selectByCondition(searchCar);
		if(CollectionUtils.isEmpty(cars)){
			return null;
		}

		for(Car tempCar : cars){
			if(Objects.equals(tempCar.getCarPlateNum(),carPlateNum)){
				return tempCar;
			}
		}
		return null;
	...
	public List<Car> getCarListByCartype(Car car) {
		ParamVerifyUtils.paramNotNull(car,"根据车型ID插叙奶分配的车，车辆对象不能为空");
		ParamVerifyUtils.paramNotNull(car.getCarTypeId(),"根据车型ID插叙奶分配的车，车型ID不能为空");
		return this.carMapper.selectCarListByCarType(car);
	}
