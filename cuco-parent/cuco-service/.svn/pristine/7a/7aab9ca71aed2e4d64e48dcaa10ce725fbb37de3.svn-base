...
public class StockServiceImpl implements StockService{
	...
	public List<Map<Object, Object>> getUsableCarTypeByUnMember(Long carportId, Long memberId, Date useCarStartTime,
		...
		try {
			...
			for(CarType carty : carport.getCarTypes()){
				...
				//根据车型ID查询可用车辆总数
				Car car =new Car();
				car.setCarTypeId(carty.getId());
				List<Car> carList= session.selectList("cn.cuco.dao.CarMapper.getAbleCarTotalByCarTypeId",car);
				int ableTotal = carList.size();
	...
	private int getAbleCarTotalAfterRestriction(CarType carType, Date useCarStartTime, int ableTotal){
		...
		List<Car> ableCarTotalList = this.carMapper.getAbleCarTotalByCarTypeId(car);
		//得到当天的限制号码
		String[] num = restrictions.split(",");
		for(String card : num){
			for(Car carsingle : ableCarTotalList){
				//查询状态是待分配和整备中的车辆
				if(carsingle.getStatus() == CarEnum.CarStatus.WAITDISTRIBUTE.getIndex() || carsingle.getStatus()  == CarEnum.CarStatus.WAITREORGANIZE.getIndex() || carsingle.getStatus()  == CarEnum.CarStatus.REORGANIZING.getIndex()){
					String carPlateNum = carsingle.getCarPlateNum();
					String lastNum = carsingle.getCarPlateNum().substring(carPlateNum.length() - 1);
	...
	@Override
	public List<Map<Object, Object>> getUsableCarType(Date useCarStartTime, Date useCarEndTime) {
		ParamVerifyUtils.paramNotNull(useCarStartTime,"用户发起用车，开始时间不能为空");
		ParamVerifyUtils.paramNotNull(useCarEndTime,"用户发起用车，结束时间不能为空");
		if(useCarStartTime.after(useCarEndTime)){
			throw new RuntimeExceptionWarn("用车开始时间不能在用车开始时间之后");
		...
		this.logger.info("发起用车开始时间是----"+DateUtils.formatDate(useCarStartTime,"yyyy-MM-dd HH:mm:ss")+",用车结束时间结束时间是----"+DateUtils.formatDate(useCarEndTime,"yyyy-MM-dd HH:mm:ss"));
		//获取车型列表
		SqlSession session=null;
		List<Map<Object, Object>> returnList = new ArrayList<>();
		...
		try {
			//获取已上架的车型列表
			List<CarType> carTypes = this.carTypeService.getCarTypeListByUp();
			if(CollectionUtils.isEmpty(carTypes)){
				throw new RuntimeExceptionWarn("没有查询到车型信息");
			}
			session=sqlSessionFactoryBean.getObject().openSession();
			useCarStartTime = chechMinutes(useCarStartTime);
			this.logger.info("查询库存开始时间规则化之后的时间是----"+DateUtils.formatDate(useCarStartTime,"yyyy-MM-dd HH:mm:ss"));
	...
	@Override
	public boolean isExitStock(Long carTypeId, Date useCarStartTime, Date useCarEndTime) {
	...
	@Override
	public List<Map<String, Object>> getRepertoryByDayForCarType(Long carTypeId, Date useCarStartDay, Date useCarEndDay) {
		ParamVerifyUtils.paramNotNull(useCarStartDay,"根据车库查询库存分布情况，开始日期不能为空");
		ParamVerifyUtils.paramNotNull(useCarEndDay,"根据车库查询库存分布情况，结束日期不能为空");
		this.logger.info("查询库存开始日期是----"+useCarStartDay+"结束日期是----"+useCarEndDay);
		if(useCarStartDay.after(useCarEndDay)){
		...
		try {
			...
			for(int i = 0; i < count; i++){
				...
				Car car = new Car();
				car.setCarTypeId(carTypeId);
				car.setStartTime(useCarStartDayMax);
				int total = session.selectOne("cn.cuco.dao.CarMapper.getCarTotalByCarType", car);
				//根据车型ID查询车型下面所有不可用的车辆总数
				int unabletotal = session.selectOne("cn.cuco.dao.CarMapper.getUnableCarTotalBycarType", car);
				Map<String, Object> countMap = new HashMap<String, Object>();
				CarOperatePlan carOperatePlan = new CarOperatePlan();
				...
				int planSize = session.selectOne("cn.cuco.dao.CarOperatePlanMapper.getCarPlanTotalByCarTypeId", carOperatePlan);
				countMap.put("total",total);
				countMap.put("usedtotal",planSize);
				countMap.put("unabletotal",unabletotal);
				countMap.put("freetotal",total-planSize-unabletotal);
				returnMap.put(DateUtils.formatDate(useCarStartDay),countMap);
				//循环结束之后日期都+1
				useCarStartDay=DateUtils.addDays(useCarStartDay, 1);
				useCarStartDayMax=DateUtils.addDays(useCarStartDayMax, 1);
