...
public class MemberAccountServiceImpl implements MemberAccountService {
	...
	public MemberAccount updateBalanceForBackstage(MemberAccountLog memberAccountLog) {
		...
		if(transformType.intValue() == 0){
			...
			memberAccountLog.setTotal(total.negate());
		}
	...
	public MemberAccount updateMemberAccountForSettlement(OrderCarUsed orderCarUsed) {
		...
		this.memberAccountMapper.updateByPrimaryKeySelective(target);
		MemberAccountLog log = new MemberAccountLog(account.getId(), order.getMemberId(), 1, AccountOperateType.SYSTEM_DEDUCTION.getIndex(), 1, order.getTotal().negate(), remark, date, orderCarUsed.getModifer(), orderCarUsed.getModifierId());
	...
	public PageResult<MemberAccountLog> getMemberAccountLogListByPage(MemberAccountLog memberAccountLog) {
		...
		Integer totalSize = this.memberAccountLogMapper.getMemberAccountLogCount(memberAccountLog);
		PageHelper.startPage(page, pageSize);
		List<MemberAccountLog> list = this.memberAccountLogMapper.getMemberAccountLogListWithSort(memberAccountLog);
	...
	public MemberAccount updateMemberAccountForFrozenDeposit(MemberAccount memberAccount) {
		...
		Long modifierId = memberAccount.getModifierId();
		String modifer = memberAccount.getModifer();
		ParamVerifyUtils.paramNotNull(memberId,"冻结用户余额作为押金时，用户ID不能为空");
		ParamVerifyUtils.paramNotNull(modifierId,"冻结用户余额作为押金时，操作人ID不能为空");
		ParamVerifyUtils.paramNotNull(modifer,"冻结用户余额作为押金时，操作人不能为空");
		BigDecimal deposit = memberAccount.getDeposit();
		if(null == deposit || deposit.doubleValue() <= 0){
			ExceptionUtil.throwWarn("冻结用户余额作为押金时，冻结押金金额不合法");
		}
		MemberAccount account = this.getMemberAccountByMemberId(memberId);
		//更新账户
		MemberAccount target = new MemberAccount();
		target.setBalance(account.getBalance().subtract(deposit));
		target.setId(account.getId());
		target.setModifer(modifer);
		target.setModifierId(modifierId);
		this.memberAccountMapper.updateByPrimaryKeySelective(target);
		//添加日志
		String remark = "用车超过27天，冻结余额作为押金";
		MemberAccountLog log = new MemberAccountLog(account.getId(), memberId, 1, AccountOperateType.FROZEN_DEPOSIT.getIndex(), 1, deposit.negate(), remark, new Date(), modifer, modifierId);
		this.memberAccountLogMapper.insertSelective(log);
		return this.getMemberAccountByMemberId(memberId);
	}
	...
	public MemberAccount updateMemberAccountForSystemBack(MemberAccount memberAccount) {
		...
		Long modifierId = memberAccount.getModifierId();
		String modifer = memberAccount.getModifer();
		String remark = memberAccount.getRemark();//备注
		ParamVerifyUtils.paramNotNull(memberId,"系统退回余额时，用户ID不能为空");
		ParamVerifyUtils.paramNotNull(modifierId,"系统退回余额时，操作人ID不能为空");
		ParamVerifyUtils.paramNotNull(modifer,"系统退回余额时，操作人不能为空");
		ParamVerifyUtils.paramNotNull(remark,"系统退回余额时，备注不能为空");
		BigDecimal balance = memberAccount.getBalance();
		if(null == balance || balance.doubleValue() <= 0){
			ExceptionUtil.throwWarn("冻结用户余额作为押金时，冻结押金金额不合法");
		}
		MemberAccount account = this.getMemberAccountByMemberId(memberId);
		//更新账户
		MemberAccount target = new MemberAccount();
		target.setBalance(account.getBalance().add(balance));
		target.setId(account.getId());
		target.setModifer(modifer);
		target.setModifierId(modifierId);
		this.memberAccountMapper.updateByPrimaryKeySelective(target);
		//添加日志
		remark = modifer + ":" +remark;
		MemberAccountLog log = new MemberAccountLog(account.getId(), memberId, 1, AccountOperateType.SYSTEM_BACK.getIndex(), 0, balance, remark, new Date(), modifer, modifierId);
		this.memberAccountLogMapper.insertSelective(log);
		return this.getMemberAccountByMemberId(memberId);
	}
	
	public static void main(String[] args) {
		System.out.println(new BigDecimal(100).add(new BigDecimal(-400)));
		System.out.println(new BigDecimal(-2100).abs());
	}

	@Override
	public MemberAccount updateMemberAccountBalance(MemberAccountLog memberAccountLog) {
		ParamVerifyUtils.paramNotNull(memberAccountLog);
		Long memberId = memberAccountLog.getMemberId();
		String modifer = memberAccountLog.getModifer();
		Long modifierId = memberAccountLog.getModifierId();
		ParamVerifyUtils.paramNotNull(memberId,"修改账户余额时，用户ID不能为空");
		ParamVerifyUtils.paramNotNull(modifer,"修改账户余额时，操作人不能为空");
		ParamVerifyUtils.paramNotNull(modifierId,"修改账户余额时，操作人ID不能为空");
		ParamVerifyUtils.paramNotNull(memberAccountLog.getOperateType(),"修改账户余额时，操作类型不能为空");
		ParamVerifyUtils.paramNotNull(memberAccountLog.getRemark(),"修改账户余额时，备注不能为空");
		ParamVerifyUtils.paramNotNull(memberAccountLog.getTransformType(),"修改账户余额时，变动类型不能为空");
		BigDecimal total = memberAccountLog.getTotal();
		ParamVerifyUtils.paramNotNull(total,"修改账户余额时，变动金额不能为空");
		total = total.abs();
		if(AccountTransformType.SPEND.getIndex().equals(memberAccountLog.getTransformType())){
			total = total.negate();
		}
		MemberAccount account = this.getMemberAccountByMemberId(memberId);
		//修改账户
