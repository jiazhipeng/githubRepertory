...
//				this.setMemberCarportList(cur);
	...
	private void setMemberCarUsedInfo(Member search,Member member){
		//用车次数
		Integer count = this.memberCarUsedService.getOrderMemberCarUsedCountByMemberId(member.getId());
		if(null != count){
		...
		//最后用车时间
		OrderMemberCarUsed used = new OrderMemberCarUsed();
		used.setMemberId(member.getId());
		used.setStartDate(search.getUseCarStartDate());
		used.setEndDate(search.getUseCarEndDate());
		used = this.memberCarUsedService.getLastOrderMemberCarUsedByMemberId(used);
		if(null != used){
