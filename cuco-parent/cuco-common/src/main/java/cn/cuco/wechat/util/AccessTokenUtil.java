...
public class AccessTokenUtil {
	...
	public final static String TOKENKEY = "accesstoken_hy_security";
	...
	* @return String
	...
	public static synchronized String getAccessToken(){
		...
		Object accesstoken = RelyJedis.getObject(TOKENKEY);
		if(accesstoken != null){
			logger.info("从 redis 获取 accesstoken>>>>>>>>>>>>>"+accesstoken.toString());
			logger.info("从 redis 获取 accesstoken-generator>>>>>>>>>>>>>"+RelyJedis.getObject(tokenKeyGenerator));
		...
		try {
			...
			RelyJedis.setObject(TOKENKEY, tokenInit, 7200);
			
			RelyJedis.setObject(tokenKeyGenerator, tokenInit+"["+Constant.APPID+" __ "+Constant.APPSECRET+" __ "+RelyDate.formatDateTime(new Date())+" __ "+"]", 7200);
			accesstoken = RelyJedis.getObject(TOKENKEY);
...
//		String accessTokenMD5 = MD5Util.MD5Encode(accessToken);
		String ticketStr = RelyJedis.get(accessToken);
		...
		if(ticketStr == null || "".equals(ticketStr)){
			...
			RelyJedis.set(accessToken, ticket, 7200);
			ticketStr = RelyJedis.get(accessToken);
	...
	public static String refreshAccessToken(){
		...
		if(token != null){
			RelyJedis.setObject(TOKENKEY, token, 7200);
			return token;
		}
		return null;
	}
	...
	/**
	 * 
	* @Description:通过code换取AccessToken,包含openid，access_token，refresh_token    
	* @author:1mobility  
