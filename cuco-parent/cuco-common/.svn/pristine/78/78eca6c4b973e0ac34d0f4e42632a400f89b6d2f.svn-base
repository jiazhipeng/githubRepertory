...
/**
 * 
* description：通用weChatUrl    
* author：1mobility    
* time：2017年2月20日 下午4:28:11    
* 修改时间：2017年2月20日 下午4:28:11    
* 修改备注：
 */
public class WechatUrl {
	...
	/**
	 * 
	* @Title: getUrlBySnsapibase 
	* @Description: 静默授权
	* @author jiaxiaoxian
	* @param reirectUrl 回调的url,返回获取code和state
	* @param state  自己设置的state，回调微信会给带回来
	* @throws Exception
	* @return String
	 */
	public static String getUrlBySnsapibase(String reirectUrl,String state) throws Exception {
		String weixinurl = AUTHORIZE_URL;
		weixinurl = weixinurl.replace("{appid}", Constant.APPID);
		weixinurl = weixinurl.replace("{redirect_uri}", URLEncoder.encode(reirectUrl, "UTF-8"));
		weixinurl = weixinurl.replace("{state}", state);
	...
	* @return String
	 */
	public static String getUrl(String reirectUrl,String state) throws Exception {
	 ...
	 * @param targetUrl
	 * @param method
	 * @param param post请求发送的时候需要传param
	 * @return
	 * @throws IOException
	 */
	public static String getRemoteJson(String targetUrl, String method,String param)
			throws IOException {
		URL uri = new URL(targetUrl);
		String userAgent = "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.66 Safari/537.36";// 模拟浏览器
		HttpURLConnection connection = (HttpURLConnection) uri.openConnection();
	...
	public static Map<String, String> FetchConfigParams(String apiCalledUrl,
		...
		try {
			...
			String ticket = getJSApiTicket(AccessTokenUtil.getAccessToken());
